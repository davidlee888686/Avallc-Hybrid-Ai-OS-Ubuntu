...
def collect_dependency_status() -> dict[str, Any]:
    tools = {
        ...
        "python3-venv": shutil.which("python3") is not None,
...
def detect_linux_family() -> str:
    ...
    if "id_like=debian" in text or "id=ubuntu" in text or "id=debian" in text:
        return "debian"
    if "id_like=fedora" in text or "id=fedora" in text or "id=rhel" in text:
        return "fedora"
    return "unknown"


def install_commands_for_family(family: str) -> list[list[str]]:
    if family == "debian":
        return [
            ["sudo", "apt-get", "update"],
            ["sudo", "apt-get", "install", "-y", "python3", "python3-venv", "python3-pip", "git", "poppler-utils", "binutils"],
        ]
    if family == "fedora":
        return [
            ["sudo", "dnf", "install", "-y", "python3", "python3-pip", "git", "poppler-utils", "binutils"],
        ]
    return []
...
def cmd_setup_env(args: argparse.Namespace) -> int:
    ...
    commands = install_commands_for_family(family)
    ...
    if args.apply and commands:
        for cmd in commands:
            ...
            if proc.returncode != 0:
                break
        payload["dependency_status_after"] = collect_dependency_status()
    ...
    print(f"Detected Linux family: {family}")
    if not commands:
        print("No known install commands for this platform. Install python3, git, pdftotext, and strings manually.")
    else:
        print("Planned install commands:")
        for c in commands:
            print(f"- {' '.join(c)}")
    ...
    if args.apply:
        if payload["results"]:
            for r in payload["results"]:
                print(f"- ran: {r['command']} (rc={r['returncode']})")
        else:
            print("- apply requested, but no install commands were available for this platform")
...
DEFAULT_NEXT_STEPS = [
    ...
    "If dependencies are missing: setup-env (or setup-env --apply on Ubuntu/Debian)",
...
FRESH_INSTALL_STEPS = [
    "cd ~/code",
    "git clone https://github.com/davidlee888686/Avallc-Hybrid-Ai-OS-Ubuntu.git",
    "cd Avallc-Hybrid-Ai-OS-Ubuntu",
...
def cmd_start_here(args: argparse.Namespace) -> int:
    payload = {
        "mode": args.mode,
        "steps": FRESH_INSTALL_STEPS if args.mode == "fresh-install" else DEFAULT_NEXT_STEPS,
        "notes": [
            "If SSH clone fails with publickey, use HTTPS clone.",
            "Do not use placeholder text like <YOUR_USERNAME> in terminal commands.",
        ],
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_setup = sub.add_parser("setup-env", help="Show or run OS package install commands for required tools")
    p_setup.add_argument("--apply", action="store_true", help="Actually run install commands (uses sudo)")
    p_setup.add_argument("--output", choices=["text", "json"], default="text")
    ...
    p_start = sub.add_parser("start-here", help="Show exact next commands (fresh install or build mode)")
    p_start.add_argument("--mode", choices=["fresh-install", "build"], default="fresh-install")
    p_start.add_argument("--output", choices=["text", "json"], default="text")
