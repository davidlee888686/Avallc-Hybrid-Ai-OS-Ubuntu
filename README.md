...
Install base dependencies first (Python, Git, PDF tools):

```bash
python3 tools/aava_migration.py setup-env
```

Apply install commands automatically on Ubuntu/Debian/Fedora (uses sudo):

```bash
python3 tools/aava_migration.py setup-env --apply
```
...
### Start here (fresh install on a new machine)

```bash
python3 tools/aava_migration.py start-here --mode fresh-install
python3 tools/aava_migration.py start-here --mode fresh-install --output json
```

This prints exact clone/setup/build-prep commands with your repo path prefilled and no placeholder tokens.
